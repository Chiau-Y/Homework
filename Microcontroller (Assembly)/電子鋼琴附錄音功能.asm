...
; -------------------------------------------------------------------------------- ;
;  1.偵測按鍵-TIMER1+中斷
;      (1)設TIMER1為16位元非同步計數器模式，並關閉前除器
;      (2)使用外部32768Hz震盪器，並將3276傳送給TMR1H和TMR1L
;      (3)每0.1秒中斷一次，檢查是按哪個按鍵
;      (4)設為高優先中斷
;  2.以電腦鍵盤當琴鍵-RS-232通訊+中斷
;      (1)利用ASCII編碼符號，傳遞文字資料
;      (2)用RS-232，進行微處理器與電腦之間的資料傳輸
;      (3)設為低優先中斷
;  3.由蜂鳴器傳出八度音-PWM+TIMER2
;      (1)設CCP1為PWM模式，並設工作週期最低2位元為00
;      (2)啟動TIMER2，並設其前除器為16倍
;      (3)定義好音名和其PR2值，算式為PWM period=[PR2+1]*4*10^(-6)*16，數值如表(一)
...
;      (3)LED的bit0亮代表錄音開始
;      (4)LED從bit0往bit7方向亮，全亮時代表結束錄音
;  5.按SW4，播放錄音-A/D轉換
;      (1)當利用暫存器確認錄過音後，及開啟A/D轉換
;      (2)測AN2通道的電壓是否為0，0則代表按下SW4
;      (3)利用FSR，將110個暫存器的值，放入PR2，並給相對應的CCPR1L值
;      (4)LED從bit7往bit0方向滅，全滅時代表結束播放音樂
; -------------------------------------------------------------------------------- ;

list p=18f4520 ; PIC18 F4520
